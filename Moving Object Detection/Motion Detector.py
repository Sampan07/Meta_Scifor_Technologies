...
# Create an empty mask for drawing keypoints
...
while True:
    ...
    # Apply Gaussian blur to the energy function to reduce noise
    energy_function_blurred = cv2.GaussianBlur(energy_function, (35, 35), 0)

    # Apply thresholding to obtain a binary mask
      # Adjust this threshold as needed
    _, binary_mask = cv2.threshold(energy_function_blurred, threshold_value, 255, cv2.THRESH_BINARY)
    ...
    for contour in contours:
        if cv2.contourArea(contour) > min_contour_area:  # Adjust this threshold as needed
            ...
            cv2.putText(frame, 'motion detected', (x, y - 3), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1)
